    ...
    fn op_in(&mut self) -> Result<FrameControl<'gc>, Error<'gc>> {
        ...
        let has_prop = match value {
            ...
            _ => {
                ...
                // Primitives don't have any accessible properties on themselves,
                // but we do still need to check the prototype
                if let Some(proto) = value.proto(self) {
                    proto.has_property(&multiname)
                } else {
                    // (technically, this condition can't be met, since
